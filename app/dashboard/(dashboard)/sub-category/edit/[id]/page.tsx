...
export default function CategoryCreate() {
    ...
    useEffect(() => {
        async function getData() {
            ...
            const subCategoryRes = await fetch(apiUrl('/sub-category'), {
                method: "PUT",
                body: JSON.stringify({ id })
            });
            const subCategoryJson = await subCategoryRes.json();

            if (subCategoryJson.ok) {
                form.setValue("category", id as string);
                form.setValue("status", subCategoryJson.data.status);
                form.setValue("title", subCategoryJson.data.title);
            }
            console.log(id);
    ...
    return (
        <div className="border p-4 bg-accent/50 rounded-lg">
            <div className="border mb-4 p-4 rounded-xl flex justify-between">
                <h2 className="text-xl font-semibold">Update sub category</h2>
